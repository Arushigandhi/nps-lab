...
int main()
{
	...
	while (1)
	{
		...
		case 1: // asc order
			...
			for (int i = 0; i < sz; i++)
				scanf("%d", &buffer[i]); //& comes here

			send(sockfd, buffer, sizeof(buffer), 0); // no & here
...
// jab char and int array hota hai tab & nahi lagate send and recv me aur jab int hota hai tab lagate hai